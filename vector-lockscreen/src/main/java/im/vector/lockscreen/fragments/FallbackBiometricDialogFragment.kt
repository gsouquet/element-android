...
import im.vector.lockscreen.databinding.FragmentBiometricDialogContainerBinding
import im.vector.lockscreen.databinding.ViewBiometricDialogContentBinding
...
class FallbackBiometricDialogFragment: DialogFragment(R.layout.fragment_biometric_dialog_container) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        requireDialog().setTitle(parsedArgs.title ?: getString(R.string.sign_in_pf))
    }
    ...
    private fun renderSuccess() {
        ...
        contentBinding.fingerprintIcon.setImageResource(R.drawable.ic_fingerprint_success_pf)
        contentBinding.fingerprintStatus.apply {
            setTextColor(ResourcesCompat.getColor(resources, R.color.success_color, null))
            setText(R.string.fingerprint_success_pf)
        }
        viewLifecycleOwner.lifecycleScope.launch {
            delay(200L)
            dismiss()
    ...
    private fun renderFailure() {
        val contentBinding = binding ?: return
        contentBinding.fingerprintIcon.setImageResource(R.drawable.ic_fingerprint_error_pf)
        contentBinding.fingerprintStatus.apply {
            setTextColor(ResourcesCompat.getColor(resources, R.color.warning_color, null))
            setText(R.string.fingerprint_not_recognized_pf)
        }
        viewLifecycleOwner.lifecycleScope.launch {
            delay(1500L)
            resetState()
    ...
    private fun resetState() {
        val contentBinding = binding ?: return
        contentBinding.fingerprintIcon.setImageResource(R.drawable.ic_fp_40px_pf)
        contentBinding.fingerprintStatus.apply {
            setTextColor(ResourcesCompat.getColor(resources, R.color.hint_color, null))
            setText(R.string.fingerprint_hint_pf)
        }
    }
