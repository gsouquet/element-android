...
import im.vector.lockscreen.databinding.FragmentLockScreenBinding
import im.vector.lockscreen.utils.vibrate
import im.vector.lockscreen.views.CodeView
...
class VectorLockScreenFragment: Fragment(R.layout.fragment_lock_screen), MavericksView {
    ...
    private fun onAuthFailure(method: AuthMethod) {
        ...
        if (configuration.animateOnError) {
            context?.let {
                val animation = AnimationUtils.loadAnimation(it, R.anim.shake_pf)
    ...
    private fun setupTitleView(titleView: TextView, isConfirmation: Boolean, configuration: LockScreenConfiguration) = with(titleView) {
        text = if (isConfirmation) {
            configuration.newCodeConfirmationTitle ?: getString(R.string.confirm_pin)
        } else {
            configuration.title ?: getString(R.string.lock_screen_title_pf)
        }
    }
